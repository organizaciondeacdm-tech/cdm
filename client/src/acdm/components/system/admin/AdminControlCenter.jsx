...
export function AdminControlCenter({ section, currentUser, onNavigateSection, onSessionSwitched }) {
  ...
  const [outboxSelectedIds, setOutboxSelectedIds] = useState([]);
  const [outboxRequeueFromStatus, setOutboxRequeueFromStatus] = useState("dead_letter");
  ...
  const loadOutbox = async (options = {}) => {
    const page = Number(options.page ?? outboxPage) || 1;
    const limit = Number(options.limit ?? outboxLimit) || 25;
    const status = options.status ?? outboxStatusFilter;
    const [statsRes, eventsRes] = await Promise.all([
      apiService.getOutboxStats(),
      apiService.getOutboxEvents({ status, page, limit })
    ]);
    setOutboxStats(statsRes?.data || null);
    const payload = eventsRes?.data || {};
    setOutboxRows(Array.isArray(payload.rows) ? payload.rows : []);
    setOutboxPage(Number(payload.pagination?.page || page));
    setOutboxLimit(Number(payload.pagination?.limit || limit));
    setOutboxTotal(Number(payload.pagination?.total || 0));
  };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (["admin-outbox"].includes(section)) {
        await loadOutbox({ page: 1 });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (section !== "admin-outbox") return;
    loadOutbox({ page: 1 }).catch(() => {});
  }, [section, outboxStatusFilter, outboxLimit]);

  useEffect(() => {
    const validIds = new Set(outboxRows.map((row) => String(row?._id || "")));
    setOutboxSelectedIds((prev) => prev.filter((id) => validIds.has(String(id))));
  }, [outboxRows]);
  ...
  const outboxTotalPages = useMemo(() => {
    const total = Math.max(0, Number(outboxTotal || 0));
    const size = Math.max(1, Number(outboxLimit || 25));
    return Math.max(1, Math.ceil(total / size));
  }, [outboxTotal, outboxLimit]);

  const goOutboxPage = async (nextPage) => {
    const page = Math.min(Math.max(1, Number(nextPage || 1)), outboxTotalPages);
    try {
      await loadOutbox({ page });
    } catch (err) {
      setError(err.message || "No se pudo cargar eventos de outbox");
    }
  };

  const toggleOutboxSelection = (id) => {
    const key = String(id || "");
    setOutboxSelectedIds((prev) => (
      prev.includes(key) ? prev.filter((rowId) => rowId !== key) : [...prev, key]
    ));
  };

  const toggleSelectAllOutboxRows = () => {
    const rowIds = outboxRows.map((row) => String(row?._id || "")).filter(Boolean);
    if (rowIds.length === 0) return;
    setOutboxSelectedIds((prev) => {
      const allSelected = rowIds.every((id) => prev.includes(id));
      if (allSelected) return prev.filter((id) => !rowIds.includes(id));
  ...
  const processOutboxNow = async () => {
    setOutboxBusy(true);
    try {
      setError("");
      setNotice("");
      const response = await apiService.processOutboxNow();
      setNotice(`Outbox procesado. Procesados: ${response?.data?.processed || 0}, fallidos: ${response?.data?.failed || 0}, DLQ: ${response?.data?.deadLettered || 0}`);
      await loadOutbox({ page: 1 });
    } catch (err) {
      setError(err.message || "No se pudo procesar el outbox");
    } finally {
      setOutboxBusy(false);
    }
  };

  const requeueSelectedOutbox = async () => {
    if (outboxSelectedIds.length === 0) {
      setError("Seleccione eventos para reencolar");
      return;
    }
    if (!window.confirm(`¿Reencolar ${outboxSelectedIds.length} evento(s)?`)) return;

    setOutboxBusy(true);
    try {
      setError("");
      setNotice("");
      const response = await apiService.requeueOutbox({
        ids: outboxSelectedIds,
        fromStatus: outboxRequeueFromStatus
      });
      setOutboxSelectedIds([]);
      setNotice(`Eventos reencolados: ${response?.data?.modified || 0}/${response?.data?.requested || 0}`);
      await loadOutbox({ page: 1 });
    } catch (err) {
      setError(err.message || "No se pudo reencolar eventos");
    } finally {
      setOutboxBusy(false);
    }
  };
  ...
  return (
    <div>
      ...
      {(section === "admin-traffic" || section === "admin-security" || section === "admin-outbox") && (
        <div className="card mb-16">
          <div className="flex gap-8" style={{ flexWrap: "wrap" }}>
            ...
            <button
              className="btn btn-secondary btn-sm"
              style={{
                background: section === "admin-outbox" ? "rgba(0,212,255,0.15)" : undefined,
                borderColor: section === "admin-outbox" ? "var(--accent)" : undefined
              }}
              onClick={() => onNavigateSection?.("admin-outbox")}
            >
              📦 Outbox
            </button>
          </div>
      ...
      {section === "admin-outbox" && (
        <div className="card-grid" style={{ gridTemplateColumns: '1fr 2fr' }}>
          <div className="card">
            <div className="card-header"><span className="card-title">Estado del Worker</span></div>
            <p>Worker: <strong>{outboxStats?.enabled ? "habilitado" : "deshabilitado"}</strong></p>
            <p>Running: <strong>{outboxStats?.running ? "sí" : "no"}</strong></p>
            <p>Worker ID: <span style={{ fontSize: 12 }}>{outboxStats?.workerId || "-"}</span></p>
            <p>Batch: <strong>{outboxStats?.batchSize ?? "-"}</strong> | Poll: <strong>{outboxStats?.pollMs ?? "-"}ms</strong></p>
            <p>Max retries: <strong>{outboxStats?.maxRetries ?? "-"}</strong></p>
            <hr />
            <p>Total: <strong>{outboxStats?.counters?.total ?? 0}</strong></p>
            <p>Pending: <strong>{outboxStats?.counters?.pending ?? 0}</strong></p>
            <p>Processing: <strong>{outboxStats?.counters?.processing ?? 0}</strong></p>
            <p>Processed: <strong>{outboxStats?.counters?.processed ?? 0}</strong></p>
            <p>Dead-letter: <strong>{outboxStats?.counters?.deadLetter ?? 0}</strong></p>
            <div className="flex gap-8 mt-16">
              <button className="btn btn-primary" onClick={processOutboxNow} disabled={outboxBusy}>
                {outboxBusy ? "Procesando..." : "Procesar ahora"}
              </button>
              <button className="btn btn-secondary" onClick={() => loadOutbox({ page: 1 })} disabled={outboxBusy}>
                Recargar
              </button>
            </div>
          </div>
          ...
          <div className="card table-wrap">
            <div className="flex items-center justify-between mb-16" style={{ gap: 8, flexWrap: "wrap" }}>
              <div className="flex items-center gap-8" style={{ flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, color: "var(--text2)" }}>herramientas</span>
                <button className="btn btn-secondary btn-sm" onClick={toggleSelectAllOutboxRows} disabled={outboxRows.length === 0}>
                  seleccionar
                </button>
                <select className="form-select" style={{ width: 170 }} value={outboxRequeueFromStatus} onChange={(e) => setOutboxRequeueFromStatus(e.target.value)}>
                  <option value="dead_letter">desde dead_letter</option>
                  <option value="processed">desde processed</option>
                  <option value="processing">desde processing</option>
                  <option value="pending">desde pending</option>
                </select>
                <button className="btn btn-secondary btn-sm" onClick={requeueSelectedOutbox} disabled={outboxBusy || outboxSelectedIds.length === 0}>
                  reencolar ({outboxSelectedIds.length})
                </button>
              </div>
              <div className="flex items-center gap-8" style={{ flexWrap: "wrap" }}>
                <select className="form-select" style={{ width: 150 }} value={outboxStatusFilter} onChange={(e) => setOutboxStatusFilter(e.target.value)}>
                ...
                <select className="form-select" style={{ width: 110 }} value={outboxLimit} onChange={(e) => setOutboxLimit(Number(e.target.value))}>
                  <option value={10}>10</option>
                  <option value={25}>25</option>
                  <option value={50}>50</option>
                  <option value={100}>100</option>
            ...
            <div className="flex items-center gap-8 mb-8">
              <button className="btn btn-secondary btn-sm" onClick={() => goOutboxPage(outboxPage - 1)} disabled={outboxPage <= 1}>◀</button>
              <span style={{ fontSize: 12, color: "var(--text2)" }}>Página {outboxPage} / {outboxTotalPages} · total {outboxTotal}</span>
              <button className="btn btn-secondary btn-sm" onClick={() => goOutboxPage(outboxPage + 1)} disabled={outboxPage >= outboxTotalPages}>▶</button>
            </div>

            <table>
              <thead><tr><th style={{ width: 42 }}>Sel</th><th>Fecha</th><th>Evento</th><th>Aggregate</th><th>Status</th><th>Retries</th><th>Error</th></tr></thead>
              <tbody>
                {outboxRows.length === 0 ? (
                  <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text2)" }}>Sin eventos</td></tr>
                ) : outboxRows.map((row) => (
                  <tr key={String(row?._id || "")}>
                    <td>
                      <input
                        type="checkbox"
                        checked={outboxSelectedIds.includes(String(row?._id || ""))}
                        onChange={() => toggleOutboxSelection(row?._id)}
                      />
                    </td>
                    <td style={{ fontSize: 11 }}>{row?.occurredAt ? new Date(row.occurredAt).toLocaleString("es-AR") : "-"}</td>
                    <td style={{ fontSize: 11 }}>{row?.eventType || "-"}</td>
                    <td style={{ fontSize: 11 }}>{row?.aggregate || "-"} / {row?.aggregateId || "-"}</td>
                    <td>{row?.status || "-"}</td>
                    <td>{Number(row?.retries || 0)}</td>
                    <td style={{ fontSize: 11 }}>{row?.lastError || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
